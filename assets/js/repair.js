...
let range_024 = document.querySelector("#range_024")
let numLin024 = parseInt(document.querySelector(".numLin024").innerText)
let cookie = "Repair"
...
for (let i = 0; i < rowOrders.length; i++)
    rowOrders[i].onclick = function(event) {
        ...
        if (target === orderChange[i-1]) {
            ...
            window.location.href = "edit-repair.php"
        }
...
for (let i = 0; i < tabTr.length; i++) {
    let td = tabTr[i].getElementsByTagName("td")[7];
    if (td) {
...
saveInquiries.onclick = function () {
    let strings = parseInt(range_024.value)
    delete_cookie("stringsRepair")
    document.cookie = "stringsRepair=" + strings + ";  expires=" + now.toUTCString() + "; path=/";
    location.reload()
}


$(function () {
    let numLin = parseInt(document.querySelector(".numLin024").innerText) - 1
    let from = values[findPartial( values, "stringsRepair" )].split('=')[1]
...
document.cookie = "numLin024=" + numLin024 + ";  expires=" + now.toUTCString() + "; path=/";
